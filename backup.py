...
def perform_traceroute(scanned_hosts):
    ...
    result, unans = traceroute(list(scanned_hosts.keys()), maxttl=6, verbose=None)
    result.graph()
    result.graph(type="ps", target="")
    result.graph(target="> blahdd.svg")
    prevhop = [
            # keyb
            # for trace in result.get_trace().values()
            # for sub_trace in trace.values()
            # for keyb in sub_trace if
            # type(keyb) != bool
    ]
